##########################################################################################
####################  Rate of evolution - Petrosal lobules BM - Br  ######################
##########################################################################################
...
PL_Body.rate<-Results$Log.Median.Scalar
...
#Make plot
plotBranchbyTrait2(tree,PL_Body.rate,mode="edges", title="Log10(PL vs. BM) evolutionary rate",
...
axis(side=1, at=Tree.root.PL-15:Tree.root.PL, labels=15:Tree.root.PL,cex.axis=0.5, pos=-0.5,
     mgp=c(2, 0.2, 0), tck=-0.02)

#label branches
edgelabels(round(PL_Body.rate,digits =2), cex=0.3, frame="none", adj = c(0.9, -0.7) )

############################    PL - Endocrania volume    ################################
#open rates
...
PL_Brain.rate<-Results$Log.Median.Scalar
...
#Make plot
plotBranchbyTrait2(tree,PL_Brain.rate,mode="edges", title="Log10(PL vs. Br) evolutionary rate",
                  palette=colorRampPalette(c("black","mediumblue","skyblue","orange","goldenrod1","lightgoldenrod1")),
                  edge.width=2.5, cex=0.3, prompt=FALSE)
...
#label branches
edgelabels(round(PL_Brain.rate,digits =2), cex=0.3, frame="none", adj = c(0.9, -0.7) )
