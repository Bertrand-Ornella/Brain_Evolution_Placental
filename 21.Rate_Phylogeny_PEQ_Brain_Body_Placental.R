##########################################################################################
####################  Rate of evolution - PEQ, Brain and Body mass  ######################
##########################################################################################
...
#tree root = 201.7734 (good for PEQ, Brain and Body mass) - see "2.read_consensus_mrbayes"
...
PEQ.rate<-Results$Log.Median.Scalar
...
#Make plot
plotBranchbyTrait2(tree,PEQ.rate,mode="edges", title="PEQ evolutionary rate",
...
#label branches
edgelabels(round(PEQ.rate,digits =2), cex=0.3, frame="none", adj = c(0.9, -0.7) )
...
Brain.rate<-Results$Log.Median.Scalar
...
#Make plot
plotBranchbyTrait2(tree,Brain.rate,mode="edges", title="Endocranial volume evolutionary rate",
                  palette=colorRampPalette(c("black","mediumblue","skyblue","orange","goldenrod1","lightgoldenrod1")),
                  edge.width=2.5, cex=0.3, prompt=FALSE)
...
#label branches
edgelabels(round(Brain.rate,digits =2), cex=0.3, frame="none", adj = c(0.9, -0.7) )
