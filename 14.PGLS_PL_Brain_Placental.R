...
##########################################################################################
####################  Petrosal lobules - PGLS, permutation tests, plots   #################
##########################################################################################
...
PL<-dataPL[c(1,16)] #select column for PL brain size
...
PL.Br<-data1[c(1:5,9,11,15)] # brain size dataset
placental.data.Br<-na.omit(PL.Br)

#Check tree with data
name.check(tree_placental, placental.data.Br, data.names=NULL)

#Save dataset
write.csv(placental.data.Br,'Placental_data_final_PL_Br.csv')

#Save tree
write.nexus(tree_placental, file = "PL_Br_tree.trees")

##################################   Analyses PL   ######################################

#Open data and tree - brain size
placental.data<-read.csv("Placental_data_final_PL_Br.csv", header=T, row.names = 1)
tree_placental<-read.nexus("PL_Br_tree.trees")
...
#PL vs Brain
Lambda_Brain<-gls(PL ~ Brain.mg, data=placental.data, 
                  correlation=corPagel(value=1,phy=tree_placental), method="ML")
Brownian_Brain<-gls(PL ~ Brain.mg, data=placental.data, 
                    correlation=corBrownian(1,phy=tree_placental), method="ML")
OU_Brain<-gls(PL ~ Brain.mg, data=placental.data, 
              correlation=corMartins(1,phy=tree_placental,fixed = TRUE), method="ML")
Blomberg_Brain<-gls(PL ~ Brain.mg, data=placental.data, 
...
placental.data$Predicted_PL_Brain <- predict(Lambda_Brain)
placental.data$Residuals_PL_Brain <- residuals(Lambda_Brain)

#export dataframe
write.csv(placental.data,'Placental_data_final_PL_Br_res.csv')

################################ boxplots and permutation test ######################################

####Open data with PL residuals
placental.data<-read.csv("Placental_data_final_PL_Br_res.csv", header=T, row.names = 1)
...
sink("ANOVA_Welsh_log_PL_Brain.txt")
...
##########
#Create density plot
ggdensity(placental.data, x = "Residuals_PL_Brain",
          add = "mean", rug = TRUE,
...
placental.data<-read.csv("Placental_data_final_PL_Br_res.csv", header=T)
...
##### For PL vs. Brain size
#Create model PGLS regression line for each Group
MeslineG_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=MesG_Tree), data=MesG)
PallineG_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=PalG_Tree), data=PalG)
EoclineS_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=EocS_Tree), data=EocS)
EoclineC_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=EocC_Tree), data=EocC)

#Prepare PGLS for each group
pgls.fit.MesG <- predict(MeslineG_Br_B) #predict values 
predframe.MesG <- with(MesG, data.frame(Species, Group, Brain.mg, PL = pgls.fit.MesG))

pgls.fit.PalG <- predict(PallineG_Br_B) #predict values 
predframe.PalG <- with(PalG, data.frame(Species, Group, Brain.mg, PL = pgls.fit.PalG))

pgls.fit.EocS <- predict(EoclineS_Br_B) #predict values 
predframe.EocS <- with(EocS, data.frame(Species, Group, Brain.mg, PL = pgls.fit.EocS))

pgls.fit.EocC <- predict(EoclineC_Br_B) #predict values 
predframe.EocC <- with(EocC, data.frame(Species, Group, Brain.mg, PL = pgls.fit.EocC))

#Make graph with PGLS corrected regressions - convex hulls -- Brain/PL
ggplot(placental.data, aes(Brain.mg, PL, color = Group)) +
  ...
  labs(x = "log10(Endocranial volume)", y = "log10(Petrosal lobule volume)") +
