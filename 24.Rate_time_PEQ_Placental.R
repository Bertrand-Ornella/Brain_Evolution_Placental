...
##########################################################################################
########################   Evolutionary rate through time - PEQ #########################
##########################################################################################
...
### Open data - PEQ
Results<-read.csv("Results_PEQ_placentals.csv", header=T, row.names = 1)
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='log10 (PEQ rate of evolution)')
