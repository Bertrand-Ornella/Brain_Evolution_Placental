...
##########################################################################################
#####################   Evolutionary rate through time - OB Brain #########################
##########################################################################################
...
### Open data - OB
Results<-read.csv("Results_OB_Brain_placentals.csv", header=T, row.names = 1)
...
seqlower<-Tree.root.OB-bin_size
#Make bins #18 total
UpperBin<-seq(Tree.root.OB, 25, by=-bin_size)
LowerBin <-seq(seqlower, 15, by=-bin_size)
...
#Result contains the branch numbers inside each bin
#myDF
...
#Add dashed line for the different bins PEQ
vline.data <- data.frame(z = seq(Tree.root.OB,20, by=-bin_size))
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='Residuals[Log10 (Olfactory bulbs vs. Endocranial volume) rate]')
