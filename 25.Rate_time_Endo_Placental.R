...
##########################################################################################
###############   Evolutionary rate through time - Endocranial volume ####################
##########################################################################################
...
Results<-read.csv("Results_Brain_placentals.csv", header=T, row.names = 1)
...
#edgelengtharray
write.csv(edgelengtharray,'time_Brain.csv')
...
write.csv(Table2,'Rate_Brain_timebins.csv')
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='log10 (Endocranial volume rate of evolution)')
