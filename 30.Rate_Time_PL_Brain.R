...
##########################################################################################
#####################   Evolutionary rate through time - PL Brain ########################
##########################################################################################
...
### Open data - PL
Results<-read.csv("Results_PL_Brain_placentals.csv", header=T, row.names = 1)
...
seqlower<-Tree.root.PL-bin_size
#Make bins #18 total
UpperBin<-seq(Tree.root.PL, 25, by=-bin_size)
...
#Add dashed line for the different bins 
vline.data <- data.frame(z = seq(Tree.root.PL,20, by=-bin_size))
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='Residuals[Log10 (Petrosal lobules vs. Endocranial volume) rate]')
