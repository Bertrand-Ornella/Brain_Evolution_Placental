...
##########################################################################################
#####################   Evolutionary rate through time - OB Body #########################
##########################################################################################
...
### Open data - OB
Results<-read.csv("Results_OB_Body_placentals.csv", header=T, row.names = 1)
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='Residuals[Log10 (Olfactory bulbs vs. Body mass) rate]')
