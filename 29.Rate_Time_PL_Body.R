...
##########################################################################################
#####################   Evolutionary rate through time - PL Body #########################
##########################################################################################
...
### Open data - PL
Results<-read.csv("Results_PL_Body_placentals.csv", header=T, row.names = 1)
...
ggplot(Table2, aes(x=Average_Bin,y=Mean))+
  ...
  labs(x='Millions of years ago', y='Residuals[Log10 (Petrosal lobules vs. Body mass) rate]')
